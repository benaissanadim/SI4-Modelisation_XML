...
<xsl:variable name="dateDebutDay" select="substring($dateDebut, 9, 2)" />
    ...
    <h1>Liste des séjours disponibles du <xsl:value-of select="$dateDebut" /> au <xsl:value-of select="$dateFin" /></h1>
    ...
    <p> <span> <xsl:text>Destination :  </xsl:text> </span>
...
<p> <span>Langue enseignée : </span>
    <xsl:for-each select="LANGUES_ENSEIGNEES/LANGUE_ENSEIGNEE">
      ...
      <p> <span>Type de séjour: </span><xsl:value-of select="@type"/></p>
    <p> <span> <xsl:text>Public destinés :  </xsl:text></span>
      <xsl:value-of select="string-join(TYPES_PUBLIC/TYPE_PUBLIC, ', ')"/> </p>
      
            <p> <span>Description: </span> <xsl:value-of select="DESCRIPTION"/></p>
         <p> <span> Activités durant le séjour :</span>
                <xsl:for-each select="ACTIVITEES/ACTIVITEE[not(@type = preceding-sibling::ACTIVITEE/@type)]">
                <div class="activites">

        <act><xsl:value-of select="@type"/></act>
        <ul>
      ...
      <p> <span>Tarif: </span><xsl:value-of select="TARIF"/></p>

      <xsl:variable name="sessions" select="SESSIONS/SESSION"/>

      <p> <span>Sessions disponibles : </span></p>
...
<h4>
Ce séjour est proposé par l'agence : <xsl:text/>
